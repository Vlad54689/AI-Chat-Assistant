...
#1) Core Workflow
#2) Data Collection: Raw messages are collected from Telegram via API.
#3) Context Retrieval: User preferences are extracted from a PostgreSQL database to parameterize the analysis.
#4) Analysis (AI Agent): A local LLM (running via Ollama) interprets the message, classifying or summarizing it based on the retrieved context.
#5) Action: The system generates a personalized summary or takes action (e.g., calendar updates).
